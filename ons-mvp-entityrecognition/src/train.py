...
def create_trainset(
    ...
    samples_inequalities = preproc.get_samples(
        bucket=bucket,
        path=inequalities_processed_path
    )
    print('samples for inequalities model collected')
    
    samples_inequalities_incremental = preproc.get_samples(
        bucket=bucket,
...
def upload_image_to_ecr():
    ...
    commands = [
        'cd /tmp/python39_loaded/root/ons-mvp-entity-recognition',
...
def load_production_variant(
    ...
    try:
        obj_uri = f's3://{bucket}/{path}'
        s3_production_variant_df = wr.s3.read_parquet(obj_uri)
...
def run_training_pipeline(
    ...
    if train_from_blank:
        ...
        # training new spacy model from pt template
        print('creating new spacy model - from template pt model')
        nlp = create_spacy_model(blank=blank)

        # training spacy model over last iteration in production
        # try:
...
def prepare_production_variant(
    ...
    glue_artifact_prefix = '/tmp/python39_loaded'
    glue_artifact_file = f'root/ons-mvp-entity-recognition/artifacts/{model}'
    glue_artifact_path = f'{glue_artifact_prefix}/{glue_artifact_file}'
    nlp.to_disk(glue_artifact_path)
    print('')
    print('saved model in ', glue_artifact_path)
